    ...
    <main className="mx-auto max-w-7xl px-6 py-12 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16">
        ...
        <section>
          ...
          <p className="mt-5 text-2xl font-bold text-slate-950">{formatBDT(item.product.salePriceCents ?? item.product.priceCents)}{item.product.salePriceCents ? <span className="ml-3 text-base font-medium text-slate-400 line-through">{formatBDT(item.product.priceCents)}</span> : null}</p>
